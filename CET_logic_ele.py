...
def cet_process(input_path, score_path, output_path):
    """
    CET 초등용 Python 스크립트 로직을 함수로 감싼 함수입니다.
    인자:
        input_path  :  입력 데이터가 담긴 DB 경로
        score_path  :  규준 데이터(점수표)가 있는 DB 경로
        output_path :  최종 결과를 저장할 DB 경로
    반환:
   ...
   ##문항 전처리
    ...
    # ------------------------------------------------------
    # 흥미요인 계산 (I -> Int 로 변경)
    df['interest_S'] = df[['Int_1','Int_7','Int_13','Int_19','Int_25','Int_31','Int_37','Int_43']].sum(axis=1)
    df['interest_E'] = df[['Int_2','Int_8','Int_14','Int_20','Int_26','Int_32','Int_38','Int_44']].sum(axis=1)
    df['interest_C'] = df[['Int_3','Int_9','Int_15','Int_21','Int_27','Int_33','Int_39','Int_45']].sum(axis=1)
    df['interest_R'] = df[['Int_4','Int_10','Int_16','Int_22','Int_28','Int_34','Int_40','Int_46']].sum(axis=1)
    df['interest_I'] = df[['Int_5','Int_11','Int_17','Int_23','Int_29','Int_35','Int_41','Int_47']].sum(axis=1)
    df['interest_A'] = df[['Int_6','Int_12','Int_18','Int_24','Int_30','Int_36','Int_42','Int_48']].sum(axis=1)


    # 적성요인 계산 (T -> Apt 로 변경)
    df['apti_S'] = df[['Apt_1', 'Apt_7',  'Apt_13', 'Apt_19', 'Apt_25', 'Apt_31', 'Apt_37', 'Apt_43']].sum(axis=1)
    df['apti_E'] = df[['Apt_2', 'Apt_8',  'Apt_14', 'Apt_20', 'Apt_26', 'Apt_32', 'Apt_38', 'Apt_44']].sum(axis=1)
    df['apti_C'] = df[['Apt_3', 'Apt_9',  'Apt_15', 'Apt_21', 'Apt_27', 'Apt_33', 'Apt_39', 'Apt_45']].sum(axis=1)
    df['apti_R'] = df[['Apt_4', 'Apt_10', 'Apt_16', 'Apt_22', 'Apt_28', 'Apt_34', 'Apt_40', 'Apt_46']].sum(axis=1)
    df['apti_I'] = df[['Apt_5', 'Apt_11', 'Apt_17', 'Apt_23', 'Apt_29', 'Apt_35', 'Apt_41', 'Apt_47']].sum(axis=1)
    df['apti_A'] = df[['Apt_6', 'Apt_12', 'Apt_18', 'Apt_24', 'Apt_30', 'Apt_36', 'Apt_42', 'Apt_48']].sum(axis=1)

    #직업환경요인 계산
    df['env_D'] = df[['EJ2-2', 'EJ4-1', 'EJ6-2', 'EJ8-1', 'EJ10-1', 'EJ12-2', 'EJ14-1', 'EJ16-2', 'EJ18-2', 'EJ20-1', 'EJ22-1', 'EJ24-1', 'EJ26-1', 'EJ28-1', 'EJ30-2']].sum(axis=1)
    df['env_I'] = df[['EJ2-1', 'EJ4-2', 'EJ6-1', 'EJ8-2', 'EJ10-2', 'EJ12-1', 'EJ14-2', 'EJ16-1', 'EJ18-1', 'EJ20-2', 'EJ22-2', 'EJ24-2', 'EJ26-2', 'EJ28-2', 'EJ30-1']].sum(axis=1)
    df['env_P'] = df[['EJ1-2', 'EJ3-1', 'EJ5-2', 'EJ7-1', 'EJ9-2', 'EJ11-1', 'EJ13-2', 'EJ15-1', 'EJ17-2', 'EJ19-1', 'EJ21-2', 'EJ23-1', 'EJ25-2', 'EJ27-1', 'EJ29-2']].sum(axis=1)
    df['env_T'] = df[['EJ1-1', 'EJ3-2', 'EJ5-1', 'EJ7-2', 'EJ9-1', 'EJ11-2', 'EJ13-1', 'EJ15-2', 'EJ17-1', 'EJ19-2', 'EJ21-1', 'EJ23-2', 'EJ25-1', 'EJ27-2', 'EJ29-1']].sum(axis=1)

    #선호직업요인 계산
    df['job_S'] = df[['pre_1', 'pre_7', 'pre_13', 'pre_19', 'pre_25', 'pre_31', 'pre_37', 'pre_43']].sum(axis=1)
    df['job_E'] = df[['pre_2', 'pre_8', 'pre_14', 'pre_20', 'pre_26', 'pre_32', 'pre_38', 'pre_44']].sum(axis=1)
    df['job_C'] = df[['pre_3', 'pre_9', 'pre_15', 'pre_21', 'pre_27', 'pre_33', 'pre_39', 'pre_45']].sum(axis=1)
    df['job_R'] = df[['pre_4', 'pre_10', 'pre_16', 'pre_22', 'pre_28', 'pre_34', 'pre_40', 'pre_46']].sum(axis=1)
    df['job_I'] = df[['pre_5', 'pre_11', 'pre_17', 'pre_23', 'pre_29', 'pre_35', 'pre_41', 'pre_47']].sum(axis=1)
    df['job_A'] = df[['pre_6', 'pre_12', 'pre_18', 'pre_24', 'pre_30', 'pre_36', 'pre_42', 'pre_48']].sum(axis=1)
    # ------------------------------------------------------
    ...
    df['interest_S_T'] = np.where(
        df['성별'] == 1,
        df['interest_S'].map(dict(zip(interest_S_score['score'], interest_S_score['T_score_m']))),
        df['interest_S'].map(dict(zip(interest_S_score['score'], interest_S_score['T_score_f'])))
    )

    df['interest_E_T'] = np.where(
        df['성별'] == 1,
        df['interest_E'].map(dict(zip(interest_E_score['score'], interest_E_score['T_score_m']))),
        df['interest_E'].map(dict(zip(interest_E_score['score'], interest_E_score['T_score_f'])))
    )

    df['interest_C_T'] = np.where(
        df['성별'] == 1,
        df['interest_C'].map(dict(zip(interest_C_score['score'], interest_C_score['T_score_m']))),
        df['interest_C'].map(dict(zip(interest_C_score['score'], interest_C_score['T_score_f'])))
    )

    df['interest_R_T'] = np.where(
        df['성별'] == 1,
        df['interest_R'].map(dict(zip(interest_R_score['score'], interest_R_score['T_score_m']))),
        df['interest_R'].map(dict(zip(interest_R_score['score'], interest_R_score['T_score_f'])))
    )

    df['interest_I_T'] = np.where(
        df['성별'] == 1,
        df['interest_I'].map(dict(zip(interest_I_score['score'], interest_I_score['T_score_m']))),
        df['interest_I'].map(dict(zip(interest_I_score['score'], interest_I_score['T_score_f'])))
    )

    df['interest_A_T'] = np.where(
        df['성별'] == 1,
        df['interest_A'].map(dict(zip(interest_A_score['score'], interest_A_score['T_score_m']))),
        df['interest_A'].map(dict(zip(interest_A_score['score'], interest_A_score['T_score_f'])))
    )

    #적성 T점수
    df['apti_S_T'] = np.where(
        df['성별'] == 1,
        df['apti_S'].map(dict(zip(apti_S_score['score'], apti_S_score['T_score_m']))),
        df['apti_S'].map(dict(zip(apti_S_score['score'], apti_S_score['T_score_f'])))
    )

    df['apti_E_T'] = np.where(
        df['성별'] == 1,
        df['apti_E'].map(dict(zip(apti_E_score['score'], apti_E_score['T_score_m']))),
        df['apti_E'].map(dict(zip(apti_E_score['score'], apti_E_score['T_score_f'])))
    )

    df['apti_C_T'] = np.where(
        df['성별'] == 1,
        df['apti_C'].map(dict(zip(apti_C_score['score'], apti_C_score['T_score_m']))),
        df['apti_C'].map(dict(zip(apti_C_score['score'], apti_C_score['T_score_f'])))
    )

    df['apti_R_T'] = np.where(
        df['성별'] == 1,
        df['apti_R'].map(dict(zip(apti_R_score['score'], apti_R_score['T_score_m']))),
        df['apti_R'].map(dict(zip(apti_R_score['score'], apti_R_score['T_score_f'])))
    )

    df['apti_I_T'] = np.where(
        df['성별'] == 1,
        df['apti_I'].map(dict(zip(apti_I_score['score'], apti_I_score['T_score_m']))),
        df['apti_I'].map(dict(zip(apti_I_score['score'], apti_I_score['T_score_f'])))
    )

    df['apti_A_T'] = np.where(
        df['성별'] == 1,
        df['apti_A'].map(dict(zip(apti_A_score['score'], apti_A_score['T_score_m']))),
        df['apti_A'].map(dict(zip(apti_A_score['score'], apti_A_score['T_score_f'])))
    )

    #선호직업 T점수
    df['job_S_T'] = np.where(
        df['성별'] == 1,
        df['job_S'].map(dict(zip(job_S_score['score'], job_S_score['T_score_m']))),
        df['job_S'].map(dict(zip(job_S_score['score'], job_S_score['T_score_f'])))
    )

    df['job_E_T'] = np.where(
        df['성별'] == 1,
        df['job_E'].map(dict(zip(job_E_score['score'], job_E_score['T_score_m']))),
        df['job_E'].map(dict(zip(job_E_score['score'], job_E_score['T_score_f'])))
    )

    df['job_C_T'] = np.where(
        df['성별'] == 1,
        df['job_C'].map(dict(zip(job_C_score['score'], job_C_score['T_score_m']))),
        df['job_C'].map(dict(zip(job_C_score['score'], job_C_score['T_score_f'])))
    )

    df['job_R_T'] = np.where(
        df['성별'] == 1,
        df['job_R'].map(dict(zip(job_R_score['score'], job_R_score['T_score_m']))),
        df['job_R'].map(dict(zip(job_R_score['score'], job_R_score['T_score_f'])))
    )

    df['job_I_T'] = np.where(
        df['성별'] == 1,
        df['job_I'].map(dict(zip(job_I_score['score'], job_I_score['T_score_m']))),
        df['job_I'].map(dict(zip(job_I_score['score'], job_I_score['T_score_f'])))
    )

    df['job_A_T'] = np.where(
        df['성별'] == 1,
        df['job_A'].map(dict(zip(job_A_score['score'], job_A_score['T_score_m']))),
        df['job_A'].map(dict(zip(job_A_score['score'], job_A_score['T_score_f'])))
    )

    #직업환경 T점수
    df['env_D_T'] = np.where(
        df['성별'] == 1,
        df['env_D'].map(dict(zip(env_d_score['score'],env_d_score['T_score_m']))),
        df['env_D'].map(dict(zip(env_d_score['score'],env_d_score['T_score_f'])))
    )

    df['env_I_T'] = np.where(
        df['성별'] == 1,
        df['env_I'].map(dict(zip(env_i_score['score'],env_i_score['T_score_m']))),
        df['env_I'].map(dict(zip(env_i_score['score'],env_i_score['T_score_f'])))
    )

    df['env_P_T'] = np.where(
        df['성별'] == 1,
        df['env_P'].map(dict(zip(env_p_score['score'],env_p_score['T_score_m']))),
        df['env_P'].map(dict(zip(env_p_score['score'],env_p_score['T_score_f'])))
    )

    df['env_T_T'] = np.where(
        df['성별'] == 1,
        df['env_T'].map(dict(zip(env_t_score['score'],env_t_score['T_score_m']))),
        df['env_T'].map(dict(zip(env_t_score['score'],env_t_score['T_score_f'])))
    )
    # ------------------------------------------------------

    # (6) 최종 factor 계산 및 순위
    df['factor_S'] = ((df['interest_S_T']*2) + (df['apti_S_T']*2) + df['job_S_T'])/5
    df['factor_E'] = ((df['interest_E_T']*2) + (df['apti_E_T']*2) + df['job_E_T'])/5
    df['factor_C'] = ((df['interest_C_T']*2) + (df['apti_C_T']*2) + df['job_C_T'])/5
    df['factor_R'] = ((df['interest_R_T']*2) + (df['apti_R_T']*2) + df['job_R_T'])/5
    df['factor_I'] = ((df['interest_I_T']*2) + (df['apti_I_T']*2) + df['job_I_T'])/5
    df['factor_A'] = ((df['interest_A_T']*2) + (df['apti_A_T']*2) + df['job_A_T'])/5
    ...
    df_count = df.iloc[:,8:182]
    df_count.notnull().sum(axis=1)

    df['반응적합'] = round((df_count.notnull().sum(axis=1)/174)*100, 2)

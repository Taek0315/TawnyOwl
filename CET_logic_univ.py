...
def cet_process(input_path, score_path, output_path):
    """
    CET 대학성인용 Python 스크립트 로직을 함수로 감싼 함수입니다.
    인자:
    ...
    # 흥미요인 계산 (I -> Int 로 변경)
    # 흥미요인 계산 (I -> Int 로 변경)
    df['interest_S'] = df[['Int_1','Int_7','Int_13','Int_19','Int_25','Int_31','Int_37']].sum(axis=1)
    df['interest_E'] = df[['Int_2','Int_8','Int_14','Int_20','Int_26','Int_32','Int_38']].sum(axis=1)
    df['interest_C'] = df[['Int_3','Int_9','Int_15','Int_21','Int_27','Int_33','Int_39']].sum(axis=1)
    df['interest_R'] = df[['Int_4','Int_10','Int_16','Int_22','Int_28','Int_34','Int_40']].sum(axis=1)
    df['interest_I'] = df[['Int_5','Int_11','Int_17','Int_23','Int_29','Int_35','Int_41']].sum(axis=1)
    df['interest_A'] = df[['Int_6','Int_12','Int_18','Int_24','Int_30','Int_36','Int_42']].sum(axis=1)


    # 성격
    df['person_S'] = df[['P_1','P_7','P_13','P_19','P_25']].sum(axis=1)
    df['person_E'] = df[['P_2','P_8','P_14','P_20','P_26']].sum(axis=1)
    df['person_C'] = df[['P_3','P_9','P_15','P_21','P_27']].sum(axis=1)
    df['person_R'] = df[['P_4','P_10','P_16','P_22','P_28']].sum(axis=1)
    df['person_I'] = df[['P_5','P_11','P_17','P_23','P_29']].sum(axis=1)
    df['person_A'] = df[['P_6','P_12','P_18','P_24','P_30']].sum(axis=1)
    ...
    #직업환경요인 계산
    df['env_D'] = df[['EJ2-2', 'EJ4-1', 'EJ6-1', 'EJ8-1', 'EJ10-1', 'EJ12-2', 'EJ14-1', 'EJ16-2', 'EJ18-2', 'EJ20-1', 'EJ22-1', 'EJ24-2', 'EJ26-2', 'EJ28-1', 'EJ30-1','EJ33-1', 'EJ37-1', 'EJ41-1', 'EJ45-1', 'EJ49-1']].sum(axis=1)
    df['env_I'] = df[['EJ2-1', 'EJ4-2', 'EJ6-2', 'EJ8-2', 'EJ10-2', 'EJ12-1', 'EJ14-2', 'EJ16-1', 'EJ18-1', 'EJ20-2', 'EJ22-2', 'EJ24-1', 'EJ26-1', 'EJ28-2', 'EJ30-2','EJ34-1', 'EJ38-1', 'EJ42-1', 'EJ46-1', 'EJ50-1']].sum(axis=1)
    df['env_P'] = df[['EJ1-1', 'EJ3-1', 'EJ5-2', 'EJ7-1', 'EJ9-1', 'EJ11-2', 'EJ13-2', 'EJ15-1', 'EJ17-2', 'EJ19-1', 'EJ21-2', 'EJ23-1', 'EJ25-2', 'EJ27-1', 'EJ29-2', 'EJ31-1', 'EJ35-1', 'EJ39-1', 'EJ43-1', 'EJ47-1']].sum(axis=1)
    df['env_T'] = df[['EJ1-2', 'EJ3-2', 'EJ5-1', 'EJ7-2', 'EJ9-2', 'EJ11-1', 'EJ13-1', 'EJ15-2', 'EJ17-1', 'EJ19-2', 'EJ21-1', 'EJ23-2', 'EJ25-1', 'EJ27-2', 'EJ29-1','EJ32-1', 'EJ36-1', 'EJ40-1', 'EJ44-1', 'EJ48-1']].sum(axis=1)

    #선호직업요인 계산
    df['job_S'] = df[['pre_1', 'pre_7', 'pre_13', 'pre_19', 'pre_25', 'pre_31', 'pre_37', 'pre_43','pre_49', 'pre_55']].sum(axis=1)
    df['job_E'] = df[['pre_2', 'pre_8', 'pre_14', 'pre_20', 'pre_26', 'pre_32', 'pre_38', 'pre_44','pre_50', 'pre_56']].sum(axis=1)
    df['job_C'] = df[['pre_3', 'pre_9', 'pre_15', 'pre_21', 'pre_27', 'pre_33', 'pre_39', 'pre_45','pre_51', 'pre_57']].sum(axis=1)
    df['job_R'] = df[['pre_4', 'pre_10', 'pre_16', 'pre_22', 'pre_28', 'pre_34', 'pre_40', 'pre_46','pre_52', 'pre_58']].sum(axis=1)
    df['job_I'] = df[['pre_5', 'pre_11', 'pre_17', 'pre_23', 'pre_29', 'pre_35', 'pre_41', 'pre_47','pre_53', 'pre_59']].sum(axis=1)
    df['job_A'] = df[['pre_6', 'pre_12', 'pre_18', 'pre_24', 'pre_30', 'pre_36', 'pre_42', 'pre_48','pre_54', 'pre_60']].sum(axis=1)
    ...
    df_count = df.iloc[:,8:238]
    df_count.notnull().sum(axis=1)

    df['반응적합'] = round((df_count.notnull().sum(axis=1)/230)*100, 2)

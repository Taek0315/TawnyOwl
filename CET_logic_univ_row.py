...
def cet_process(input_path, score_path, output_path):
    """
    [대학성인용] CET 스크립트를 행 단위로 수행하는 예시 함수:
    1) input_path에서 DF 읽기
    2) score_path(규준표) 읽기
    3) 각 행(iterrows) -> process_single_person() -> 결과 누적
    4) output_path로 Excel 저장
    """
    ...
    for idx, row in df_input.iterrows():
        row_res = process_single_person(
            row,
            interest_score, person_score, apti_score, job_score,
            env_p_t_score, env_d_i_score,
            major_df, joblist_df, description_df
        )
        # 필요시 원본DF key(이름,ID 등) 복사
        # row_res['응답번호'] = row['응답번호'] ...
        results.append(row_res)

    # 4) 결과 -> DataFrame -> Excel
    ...
    print(f"[행 단위 CET(대학성인용)] 완료 -> {output_path}")

...
def process_single_person(row_data,
    ...
    # interest
    map_int_m = interest_score_dict['T_score_m']
    map_int_f = interest_score_dict['T_score_f']
    ...
    # person
    map_per_m = person_score_dict['T_score_m']
    map_per_f = person_score_dict['T_score_f']  
    ...
    # (H) 순위 결정 (1~6위) — inline으로 tie-break 처리
    distance_map = {
    ...
    if len(first_place_codes) == 1:
        pass  # 동점 없음 → 그대로 사용
    elif len(first_place_codes) == 2:
        c1, c2 = first_place_codes
        dist1 = distance_map[(c1, code6)]
        dist2 = distance_map[(c2, code6)]
        ...
        if dist1 > dist2:
            ...
            d13 = distance_map[(c1, code3)]
            d23 = distance_map[(c2, code3)]
            if d13 < d23:
                new_order_top = [c1, c2]
            elif d13 > d23:
                new_order_top = [c2, c1]
        ...
        sorted_items = re_ordered

    elif len(first_place_codes) == 3:
        c1, c2, c3 = first_place_codes
        dist_c1 = distance_map[(c1, code6)]
        dist_c2 = distance_map[(c2, code6)]
        dist_c3 = distance_map[(c3, code6)]
        ...
        if len(candidates) == 1:
                ...
                d_oth = [(c, distance_map[(c, first)]) for c in others]
            ...
            if len(leftover) == 1:
                    mid_code = leftover[0]
                    dist_list = [(cd, distance_map[(cd, mid_code)]) for cd in candidates]
    ...
    def interpret_discriminant(discriminant):
    ...
    disc_level = interpret_discriminant(discriminant)

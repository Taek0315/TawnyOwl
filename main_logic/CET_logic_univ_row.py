...
def process_single_person(row_data,
                          interest_score_dict, person_score_dict, apti_score_dict, job_score_dict,
                          env_p_t_score_dict, env_d_i_score_dict,
                          major_df, joblist_df, description_df):
    """
    [대학성인용] 하나의 응답(한 행)을 받아, CET 로직 전부를 행 단위로 수행.
    row_data: pd.Series (사용자 응답 1행)
    반환: dict(해당 응답자의 모든 최종 결과)
    """

    # ====== (A) 흥미/성격/적성/직업환경/직업선호 계산 ======

    # -- 흥미(42문항) --
    # df['interest_S'] = sum of [Int_1, Int_7, Int_13, ..., Int_37]
    # df['interest_E'] = sum of [Int_2, Int_8, ... Int_38], etc
    ...
    interest_S = safe_sum('Int_', [1,7,13,19,25,31,37])
    interest_E = safe_sum('Int_', [2,8,14,20,26,32,38])
    interest_C = safe_sum('Int_', [3,9,15,21,27,33,39])
    interest_R = safe_sum('Int_', [4,10,16,22,28,34,40])
    interest_I = safe_sum('Int_', [5,11,17,23,29,35,41])
    interest_A = safe_sum('Int_', [6,12,18,24,30,36,42])

    # -- 성격(30문항) --
    # df['person_S'] = sum of P_1,7,13,19,25
    person_S = (row_data.get('P_1',0)+row_data.get('P_7',0)+row_data.get('P_13',0)+
                row_data.get('P_19',0)+row_data.get('P_25',0))
    person_E = (row_data.get('P_2',0)+row_data.get('P_8',0)+row_data.get('P_14',0)+
                row_data.get('P_20',0)+row_data.get('P_26',0))
    person_C = (row_data.get('P_3',0)+row_data.get('P_9',0)+row_data.get('P_15',0)+
                row_data.get('P_21',0)+row_data.get('P_27',0))
    person_R = (row_data.get('P_4',0)+row_data.get('P_10',0)+row_data.get('P_16',0)+
                row_data.get('P_22',0)+row_data.get('P_28',0))
    person_I = (row_data.get('P_5',0)+row_data.get('P_11',0)+row_data.get('P_17',0)+
                row_data.get('P_23',0)+row_data.get('P_29',0))
    person_A = (row_data.get('P_6',0)+row_data.get('P_12',0)+row_data.get('P_18',0)+
                row_data.get('P_24',0)+row_data.get('P_30',0))
    ...
    # -- 직업환경(50문항: EJ_1..EJ_50) --
    # 기존 스크립트는 EJn-1, EJn-2를 만든 뒤 sum(axis=1).
    # 여기서는 row 단위로 EJn-1 => (row[EJ_{n}] ==1)?1:0
    # EJn-2 => (row[EJ_{n}] ==2)?1:0
    def EJ1(n):
        return 1 if row_data.get(f'EJ_{n}', np.nan)==1 else 0
    def EJ2(n):
        return 1 if row_data.get(f'EJ_{n}', np.nan)==2 else 0

    # env_D = df[['EJ2-2','EJ4-1', ...]] => 아래처럼
    env_D = (EJ2(2)+EJ1(4)+EJ1(6)+EJ1(8)+EJ1(10)+EJ2(12)+EJ1(14)+EJ2(16)+
             EJ2(18)+EJ1(20)+EJ1(22)+EJ2(24)+EJ2(26)+EJ1(28)+EJ1(30)+
             EJ1(33)+EJ1(37)+EJ1(41)+EJ1(45)+EJ1(49))
    env_I = (EJ1(2)+EJ2(4)+EJ2(6)+EJ2(8)+EJ2(10)+EJ1(12)+EJ2(14)+EJ1(16)+
             EJ1(18)+EJ2(20)+EJ2(22)+EJ1(24)+EJ1(26)+EJ2(28)+EJ2(30)+
             EJ1(34)+EJ1(38)+EJ1(42)+EJ1(46)+EJ1(50))

    env_P = (EJ1(1)+EJ1(3)+EJ2(5)+EJ1(7)+EJ1(9)+EJ2(11)+EJ2(13)+EJ1(15)+
             EJ2(17)+EJ1(19)+EJ2(21)+EJ1(23)+EJ2(25)+EJ1(27)+EJ2(29)+
             EJ1(31)+EJ1(35)+EJ1(39)+EJ1(43)+EJ1(47))
    env_T = (EJ2(1)+EJ2(3)+EJ1(5)+EJ2(7)+EJ2(9)+EJ1(11)+EJ1(13)+EJ2(15)+
             EJ1(17)+EJ2(19)+EJ1(21)+EJ2(23)+EJ1(25)+EJ2(27)+EJ1(29)+
             EJ1(32)+EJ1(36)+EJ1(40)+EJ1(44)+EJ1(48))

    # -- 선호직업(60문항) --
    # df['job_S'] = sum of [pre_1,7,13,19,25,31,37,43,49,55]
    job_S = safe_sum('pre_', [1,7,13,19,25,31,37,43,49,55])
    job_E = safe_sum('pre_', [2,8,14,20,26,32,38,44,50,56])
    job_C = safe_sum('pre_', [3,9,15,21,27,33,39,45,51,57])
    job_R = safe_sum('pre_', [4,10,16,22,28,34,40,46,52,58])
    job_I = safe_sum('pre_', [5,11,17,23,29,35,41,47,53,59])
    job_A = safe_sum('pre_', [6,12,18,24,30,36,42,48,54,60])

    # ====== (B) T점수 매핑 ======
    gender = row_data.get('성별', np.nan)  # 1=남, 2=여

    # 미리 dict
    map_interest_m = interest_score_dict['T_score_m']
    map_interest_f = interest_score_dict['T_score_f']
    def lookup_interest(val):
        if pd.isna(gender):
            return None
        if gender==1:
            return map_interest_m.get(val, None)
        else:
            return map_interest_f.get(val, None)
    ...
    # 성격
    map_person_m = person_score_dict['T_score_m']
    map_person_f = person_score_dict['T_score_f']
    def lookup_person(val):
        if pd.isna(gender):
            return None
        if gender==1:
            return map_person_m.get(val,None)
        else:
            return map_person_f.get(val,None)
    ...
    # 적성
    map_apti_m = apti_score_dict['T_score_m']
    map_apti_f = apti_score_dict['T_score_f']
    def lookup_apti(val):
        if pd.isna(gender):
            return None
        if gender==1:
            return map_apti_m.get(val,None)
        else:
            return map_apti_f.get(val,None)
    ...
    # 직업
    map_job_m = job_score_dict['T_score_m']
    map_job_f = job_score_dict['T_score_f']
    def lookup_job(val):
        if pd.isna(gender):
            return None
        if gender==1:
            return map_job_m.get(val,None)
        else:
            return map_job_f.get(val,None)
    ...
    # 직업환경
    map_env_p_t_m = env_p_t_score_dict['T_score_m']
    map_env_p_t_f = env_p_t_score_dict['T_score_f']
    map_env_d_i_m = env_d_i_score_dict['T_score_m']
    map_env_d_i_f = env_d_i_score_dict['T_score_f']

    def lookup_env_p_t(val):
        if pd.isna(gender):
            return None
        if gender==1:
            return map_env_p_t_m.get(val,None)
        else:
            return map_env_p_t_f.get(val,None)
    def lookup_env_d_i(val):
        if pd.isna(gender):
            return None
        if gender==1:
            return map_env_d_i_m.get(val,None)
        else:
            return map_env_d_i_f.get(val,None)
    ...
    def safe_factor(i_t, p_t, a_t, j_t):
        # ((i_t*2) + p_t + (a_t*2) + j_t ) / 6
        if any(v is None for v in [i_t,p_t,a_t,j_t]):
            return None
    ...
    factor_vals = {
        'S': factor_S if factor_S else 0,
        'E': factor_E if factor_E else 0,
        'C': factor_C if factor_C else 0,
        'R': factor_R if factor_R else 0,
        'I': factor_I if factor_I else 0,
        'A': factor_A if factor_A else 0,
    }
    sorted_items = sorted(factor_vals.items(), key=lambda x: (-x[1], x[0]))

    def tie_break_top(sorted_items):
        # (기존 로직 그대로 or 간략화)
        top_score = sorted_items[0][1]
        first_codes = [c for c,sc in sorted_items if sc==top_score]
        if len(first_codes)==1:
            return sorted_items
        # (2명,3명 동점에 대한 처리)
        # 아래는 간단화; 실제론 위 스크립트 tie_break_top를 모두 복붙
        return sorted_items

    sorted_items = tie_break_top(sorted_items)
    sorted_labels = [x[0] for x in sorted_items]
    sorted_scores = [round(x[1],2) for x in sorted_items]
    rank_codes = {}
    rank_points= {}
    for i in range(6):
        rank_codes[i+1] = sorted_labels[i]
        rank_points[i+1] = sorted_scores[i]

    # ====== (E) 흥미/성격/적성/직업선호 각 순위(동점 단순무시) ======
    def none_to_zero(x):
        return x if x is not None else 0

    # 흥미
    arr_interest = [none_to_zero(interest_S_T),
    ...
    lbls_iecra = ['S','E','C','R','I','A']
    idxs = np.argsort(-np.array(arr_interest))
    int_lbls = [lbls_iecra[i] for i in idxs]
    int_scores=[round(arr_interest[i],2) for i in idxs]

    # 성격
    arr_person= [none_to_zero(person_S_T), none_to_zero(person_E_T),
                 none_to_zero(person_C_T), none_to_zero(person_R_T),
                 none_to_zero(person_I_T), none_to_zero(person_A_T)]
    idxs2=np.argsort(-np.array(arr_person))
    per_lbls=[lbls_iecra[i] for i in idxs2]
    per_scores=[round(arr_person[i],2) for i in idxs2]

    # 적성
    arr_apti=[none_to_zero(apti_S_T), none_to_zero(apti_E_T),
              none_to_zero(apti_C_T), none_to_zero(apti_R_T),
              none_to_zero(apti_I_T), none_to_zero(apti_A_T)]
    idxs3=np.argsort(-np.array(arr_apti))
    apti_lbls=[lbls_iecra[i] for i in idxs3]
    apti_scores=[round(arr_apti[i],2) for i in idxs3]

    # 직업선호
    arr_job=[none_to_zero(job_S_T), none_to_zero(job_E_T),
             none_to_zero(job_C_T), none_to_zero(job_R_T),
             none_to_zero(job_I_T), none_to_zero(job_A_T)]
    idxs4=np.argsort(-np.array(arr_job))
    job_lbls=[lbls_iecra[i] for i in idxs4]
    job_scores=[round(arr_job[i],2) for i in idxs4]

    # ====== (F) 매칭개수, 반응적합도, 변별도 ======
    # 매칭개수 = 1~6순위별로, (HFCE) 각각 i번째 순위와 동일?
    matching_counts={}
    for i in range(1,7):
        code_i = rank_codes[i]
        mc=0
    ...
    # 반응적합 = (df_count.notnull().sum(axis=1)/230)*100
    # 여기서는 row 단위로 230개 문항(Int_1..42=42개, P_1..30=30개, Apt_1..48=48개, EJ_1..50=50개, pre_1..60=60개 => 총 230)
    question_cols=[]
    for i in range(1,43):
    ...
    for c in question_cols:
        ...
        if pd.notna(val):
            answered+=1
    response_fit = round((answered/230)*100,2)

    # 변별도 = 1순위_점수 - ((2순위_점수+4순위_점수)/2)
    disc = None
    if all(k in rank_points for k in [1,2,4]):
        disc = round(rank_points[1] - ((rank_points[2]+rank_points[4])/2),2)

    def interpret_discriminant(disc):
        if disc is None:
            return None
        elif disc < 2:
            return '매우 낮음'
    ...
    disc_level = interpret_discriminant(disc)

    # ====== (G) Code1, Code2 분류 ======
    def all_under_30(*vals):
        return all(v<30 for v in vals if v is not None)

    code1=None
    if response_fit<=70:
        code1='분류불능a'
    elif disc is not None and disc<0.9:
        code1='분류불능b'
    ...
    # ====== (H) 직업환경 Code ======
    # if env_D_T..env_T_T all <30 => '분류불능d'
    ...
    def map_major(x):
        if x in ['분류불능a','분류불능b','분류불능c','분류불능']:
            return '분류불능'
        if x is None:
            return None
        return major_map.get(x,None)
    ...
    result = {
      ...
      # 성격
      'person_S': person_S, 'person_E': person_E, 'person_C': person_C,
      'person_R': person_R, 'person_I': person_I, 'person_A': person_A,

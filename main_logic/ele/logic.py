...
def process_single_person(
    ...
    map_intS_m = interest_score_dict['S']['T_score_m']
    map_intS_f = interest_score_dict['S']['T_score_f']
    interest_S_T = lookup_Tscore(interest_S, map_intS_m, map_intS_f)

    map_intE_m = interest_score_dict['E']['T_score_m']
    map_intE_f = interest_score_dict['E']['T_score_f']
    interest_E_T = lookup_Tscore(interest_E, map_intE_m, map_intE_f)

    map_intC_m = interest_score_dict['C']['T_score_m']
    map_intC_f = interest_score_dict['C']['T_score_f']
    interest_C_T = lookup_Tscore(interest_C, map_intC_m, map_intC_f)

    map_intR_m = interest_score_dict['R']['T_score_m']
    map_intR_f = interest_score_dict['R']['T_score_f']
    interest_R_T = lookup_Tscore(interest_R, map_intR_m, map_intR_f)

    
    map_intI_m = interest_score_dict['I']['T_score_m']
    map_intI_f = interest_score_dict['I']['T_score_f']
    interest_I_T = lookup_Tscore(interest_I, map_intI_m, map_intI_f)

    map_intA_m = interest_score_dict['A']['T_score_m']
    map_intA_f = interest_score_dict['A']['T_score_f']
    interest_A_T = lookup_Tscore(interest_A, map_intA_m, map_intA_f)

    # 적성
    map_aptiS_m = apti_score_dict['S']['T_score_m']
    map_aptiS_f = apti_score_dict['S']['T_score_f']
    apti_S_T = lookup_Tscore(apti_S, map_aptiS_m, map_aptiS_f)

    map_aptiE_m = apti_score_dict['E']['T_score_m']
    map_aptiE_f = apti_score_dict['E']['T_score_f']
    apti_E_T = lookup_Tscore(apti_E, map_aptiE_m, map_aptiE_f)

    map_aptiC_m = apti_score_dict['C']['T_score_m']
    map_aptiC_f = apti_score_dict['C']['T_score_f']
    apti_C_T = lookup_Tscore(apti_C, map_aptiC_m, map_aptiC_f)

    map_aptiR_m = apti_score_dict['R']['T_score_m']
    map_aptiR_f = apti_score_dict['R']['T_score_f']
    apti_R_T = lookup_Tscore(apti_R, map_aptiR_m, map_aptiR_f)

    map_aptiI_m = apti_score_dict['I']['T_score_m']
    map_aptiI_f = apti_score_dict['I']['T_score_f']
    apti_I_T = lookup_Tscore(apti_I, map_aptiI_m, map_aptiI_f)

    map_aptiA_m = apti_score_dict['A']['T_score_m']
    map_aptiA_f = apti_score_dict['A']['T_score_f']
    apti_A_T = lookup_Tscore(apti_A, map_aptiA_m, map_aptiA_f)

    # job
    map_jobS_m = job_score_dict['S']['T_score_m']
    map_jobS_f = job_score_dict['S']['T_score_f']
    job_S_T = lookup_Tscore(job_S, map_jobS_m, map_jobS_f)

    map_jobE_m = job_score_dict['E']['T_score_m']
    map_jobE_f = job_score_dict['E']['T_score_f']
    job_E_T = lookup_Tscore(job_E, map_jobE_m, map_jobE_f)

    map_jobC_m = job_score_dict['C']['T_score_m']
    map_jobC_f = job_score_dict['C']['T_score_f']
    job_C_T = lookup_Tscore(job_C, map_jobC_m, map_jobC_f)

    map_jobR_m = job_score_dict['R']['T_score_m']
    map_jobR_f = job_score_dict['R']['T_score_f']
    job_R_T = lookup_Tscore(job_R, map_jobR_m, map_jobR_f)

    map_jobI_m = job_score_dict['I']['T_score_m']
    map_jobI_f = job_score_dict['I']['T_score_f']
    job_I_T = lookup_Tscore(job_I, map_jobI_m, map_jobI_f)

    map_jobA_m = job_score_dict['A']['T_score_m']
    map_jobA_f = job_score_dict['A']['T_score_f']
    job_A_T = lookup_Tscore(job_A, map_jobA_m, map_jobA_f)

    # env
    map_envD_m = env_d_score_dict['T_score_m']
    map_envD_f = env_d_score_dict['T_score_f']
    env_D_T = lookup_Tscore(env_D, map_envD_m, map_envD_f)

    map_envI_m = env_i_score_dict['T_score_m']
    map_envI_f = env_i_score_dict['T_score_f']
    env_I_T = lookup_Tscore(env_I, map_envI_m, map_envI_f)

    map_envP_m = env_p_score_dict['T_score_m']
    map_envP_f = env_p_score_dict['T_score_f']
    env_P_T = lookup_Tscore(env_P, map_envP_m, map_envP_f)

    map_envT_m = env_t_score_dict['T_score_m']
    map_envT_f = env_t_score_dict['T_score_f']
